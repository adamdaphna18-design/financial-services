...
def fast_safe_load(stream):
    """Use CSafeLoader if available for significant performance gains."""
...
# --- 1. YAML parse ----------------------------------------------------------
# Cache parsed YAMLs to avoid redundant file I/O in reference resolution (Step 4)
...
for yml in sorted(MANAGED.rglob("*.yaml")):
    ...
    try:
        with open(yml) as f:
            parsed_managed_yamls[yml] = fast_safe_load(f)
...
def check_refs(yml: Path) -> None:
    if yml in parsed_managed_yamls:
        data = parsed_managed_yamls[yml] or {}
    else:
        try:
            data = fast_safe_load(yml.read_text()) or {}
