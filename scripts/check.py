...
try:
    ...
    try:
        # Performance optimization: CSafeLoader is 10x-50x faster than SafeLoader
...
# --- 1. YAML parse ----------------------------------------------------------
# Performance optimization: cache parsed YAMLs to avoid redundant file I/O
parsed_yamls: dict[Path, dict] = {}
for yml in sorted(MANAGED.rglob("*.yaml")):
    checked += 1
    try:
        with open(yml) as f:
            parsed_yamls[yml] = yaml.load(f, Loader=SafeLoader)
