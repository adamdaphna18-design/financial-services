...
# ⚡ Bolt: Use C-based loader for ~3x faster YAML parsing when available
try:
...
# --- 1. YAML parse ----------------------------------------------------------
# ⚡ Bolt: Cache parsed YAML to avoid redundant file I/O in step 4
yaml_cache = {}
for yml in sorted(MANAGED.rglob("*.yaml")):
    checked += 1
    try:
        with open(yml) as f:
            yaml_cache[yml] = load_yaml(f)
...
def check_refs(yml: Path) -> None:
    # ⚡ Bolt: Use cached YAML from step 1
    if yml in yaml_cache:
        data = yaml_cache[yml] or {}
    else:
        try:
            data = load_yaml(yml.read_text()) or {}
