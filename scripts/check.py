...
try:
    ...
    # Use CSafeLoader for significant performance gains when parsing YAML heavily
    try:
...
# Cache parsed YAML content to avoid redundant file I/O and parsing overhead
...
def load_yaml(path: Path) -> dict:
    ...
    if key not in _yaml_cache:
        _yaml_cache[key] = yaml.load(path.read_text(), Loader=SafeLoader)
